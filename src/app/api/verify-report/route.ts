...
async function requireAdminNasional() {
  ...
  if (!prof || prof.role !== "admin_nasional") {
    console.error(`Access denied. User role: ${prof?.role || 'null'}, Required: admin_nasional`);
    return { error: "Forbidden: Only admin_nasional can verify reports", status: 403 as const };
  }
...
export async function POST(req: Request) {
  try {
    ...
    const { report_id, action } = body; // action: 'approve' | 'reject'
    ...
    // Use server client with user session, so auth.uid() works in RPC function
    ...
    return NextResponse.json({
      success: true,
      message: `Report ${action === "approve" ? "verified" : "rejected"} successfully`,
      report: data,
    });
  } catch (e: any) {

...
export function SudahVerifikasi({ onNavigate }: SudahVerifikasiProps) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      // remove from UI immediately (same as undo)
      setRows((prev) => prev.filter((x) => x.id !== id));
      ...
      if (err instanceof Error) setErrorMsg(err.message || "Gagal menghapus laporan.");
      else setErrorMsg("Gagal menghapus laporan.");
    } finally {
      setBusyId(null);
    }
  };
